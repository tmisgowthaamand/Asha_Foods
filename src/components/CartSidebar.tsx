...
const CartSidebar = ({ isOpen, onClose }: CartSidebarProps) => {
  ...
  return (
    <>
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 bg-black/20 z-40 touch-manipulation" 
          onClick={onClose}
        />
      )}
      
      {/* Sidebar */}
      <div className={`fixed top-0 right-0 h-full w-full sm:w-96 md:w-80 bg-background border-l border-border shadow-xl transform transition-transform duration-300 z-50 ${
        ...
        <div className="flex items-center justify-between p-3 sm:p-4 border-b border-border">
          <div className="flex items-center space-x-2">
            ...
            <h2 className="font-heading font-bold text-sm sm:text-base">Cart</h2>
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4">
          {cartItems.length === 0 ? (
            <div className="text-center py-6 sm:py-8">
              ...
              <p className="text-muted-foreground text-xs sm:text-sm">Your cart is empty</p>
            </div>
          ) : (
            <div className="space-y-2 sm:space-y-3">
              {cartItems.map((item) => (
                <div key={item.id} className="flex items-start sm:items-center gap-2 sm:gap-3 p-2 sm:p-3 border border-border rounded-lg">
                  <img 
                    src={item.image} 
                    alt={item.name}
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-medium text-xs sm:text-sm leading-tight mb-1">{item.name}</h4>
                    ...
                    <div className="flex items-center gap-1 mt-2 sm:hidden">
                      ...
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-7 w-7 p-0 text-destructive ml-2 touch-manipulation"
        ...
        {cartItems.length > 0 && (
          <div className="border-t border-border p-3 sm:p-4 space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-base sm:text-lg font-heading font-bold text-primary">₹{total.toFixed(2)}</span>
            </div>
